...
def ibs_exp(x, y, month=None, year=None):  

    if y == 1:
        try:
            # ✅ SQLite connection string (creates dist_native.db in the current folder)
            connection_string = "sqlite:///.\\DBs\\dist_native\\ibs_db\\ibs.db"
            engine = create_engine(connection_string)
            metadata = MetaData()
            metadata.reflect(bind=engine)
            ibs = metadata.tables["native_ibs"]
            ...
            with engine.begin() as conn:
                    ...
                    conn.execute(
                        text("DELETE FROM native_ibs WHERE month = :month AND year = :year"),
                        {"month": month, "year": year})
                    conn.execute(text("PRAGMA incremental_vacuum;"))
                    conn.execute(ibs.insert(), x.to_dict(orient="records"))
                    conn.execute(text("PRAGMA incremental_vacuum;"))
