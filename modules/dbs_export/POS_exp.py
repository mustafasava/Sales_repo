...
def pos_exp(x, y, month=None, year=None):

    if y == 1:
        try:
            # ✅ SQLite connection string (creates dist_native.db in the current folder)
            connection_string = "sqlite:///.\\DBs\\dist_native\\pos_db\\pos.db"
            engine = create_engine(connection_string)
            metadata = MetaData()
            metadata.reflect(bind=engine)
            pos = metadata.tables["native_pos"]
            ...
            with engine.begin() as conn:
                    ...
                    conn.execute(
                        text("DELETE FROM native_pos WHERE month = :month AND year = :year"),
                        {"month": month, "year": year})
                    conn.execute(text("PRAGMA incremental_vacuum;"))
                    conn.execute(pos.insert(), x.to_dict(orient="records"))
                    conn.execute(text("PRAGMA incremental_vacuum;"))
